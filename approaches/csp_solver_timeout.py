"""
CSP Solver — Anytime variant

Same backtracking engine as csp_solver.py, extended with a time budget.
Continues searching after the first solution to maximise the soft score,
and returns the best solution found before the timeout.
"""
...
class CSPSolver:
    """
    Anytime backtracking solver with pluggable heuristics.

    use_mrv            : Minimum Remaining Values
    use_degree         : Static Degree tie-breaker
    use_dynamic_degree : Dynamic Degree tie-breaker (more precise, more expensive)
    use_lcv            : Least Constraining Value ordering
    use_fc             : Forward Checking
    ...
    def __init__(
        ...
        use_dynamic_degree: bool = False, use_lcv: bool = False, use_fc: bool = False,
        use_ac3: bool = False, time_limit: float = 20.0,
    ):
    ...
    def solve(self) -> "Schedule | None":
        """Run the anytime search and return the best Schedule found within the time budget."""
        schedule           = Schedule(self.problem)
        self.start_time    = time.time()
        self.best_solution = None
        self.best_score    = -float('inf')

        print(f"Beginning the search (timeout: {self.time_limit}s)...")

        try:
            self._backtrack(schedule)
        except TimeoutError:
            print(f"\nTimeout ({self.time_limit}s).")

        if self.best_solution:
            print(
                f"Best solution found: soft score = {self.best_score:.3f}\n"
                f"  (nodes: {self.nodes_explored}, backtracks: {self.backtrack_count})"
            )
    ...
    def _backtrack(self, schedule: Schedule) -> bool:
        """Recursive backtracking; raises TimeoutError when the budget is exhausted."""

        if time.time() - self.start_time > self.time_limit:
            raise TimeoutError()

        if schedule.is_complete():
            current_score = self.checker.soft_score(schedule)
            ...
            # Force backtracking to keep searching for a better solution
            return False
...
if __name__ == "__main__":
    ...
    solver   = CSPSolver(problem, use_mrv=True, use_degree=True, use_lcv=True, use_fc=True, time_limit=30.0)
    start    = time.time()
    ...
    if schedule:
        ...
        print(f"Best solution found in {elapsed:.2f}s (30s budget)")
        ...
        print(f"No solution found within time limit ({elapsed:.2f}s)")

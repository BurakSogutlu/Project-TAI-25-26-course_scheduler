...
class CSPSolver:
    ...
    time_limit         : Timeout in seconds for the Anytime Algorithm (COP optimisation)
    """

    def __init__(
        self, problem: CourseScheduleProblem, use_mrv: bool = False, use_degree: bool = False, 
        use_dynamic_degree: bool = False,  use_lcv: bool = False, use_fc: bool = False, use_ac3: bool = False, time_limit: float = 20.0):
        ...
        # Metrics for constraint optimisation
        self.start_time = 0.0
    ...
    def solve(self) -> "Schedule | None":
        """
        Public entry point. Anytime Algorithm initialisation.
        """
        ...
        # Return the best found solution whithin the time given
        if self.best_solution:
            print(
                f" Best solution found : Soft Score = {self.best_score:.3f}\n"
                f"  (Explored nodes : {self.nodes_explored}, Backtracks : {self.backtrack_count})"
            )
    ...
    def _backtrack(self, schedule: Schedule) -> bool:
        """Recursive backtracking with all optional heuristics and Anytime logic."""

        # Stop if timeout
        if time.time() - self.start_time > self.time_limit:
            raise TimeoutError()

        # Anytime Algorithm : Evaluation 
        if schedule.is_complete():
            ...
            if current_score > self.best_score:
                ...
                print(f"  -> New solution found ! Score = {current_score:.3f}")
                
                # If perfect score : early stop
                if current_score == 1.0:
                    return True 
            
            # Return false to foce the algorithm to backtrack and find a better solution
            return False
